...
def main():
    ...
    #         best_model_seed = list(np.load(f))
    #     with open(os.path.join(log_dir, "sharpe_test.npy"), "rb") as f:
    #         sharpe_list_test = list(np.load(f))
    #     with open(os.path.join(log_dir, "sharpe_eval.npy"), "rb") as f:
    #         sharpe_list_eval = list(np.load(f))
    #     last_step = start_step - 1
    #     best_seed = best_model_seed[last_step]
    #     start_date = find_closest_date_after(start_date_list_train[last_step], date_list)
    #     end_date = find_closest_date_before(end_date_list_train[last_step], date_list)
    #     best_model_save_path = os.path.join(log_dir, f"best_model_{last_step}")
    #     to_add_best=f"_seed={best_seed}, {start_date=}, {end_date=}, step={last_step}"
    ...
    for step in range(start_step, len(start_date_list_train)):
        ...
        for seed in seeds:
            ...
            eval_env_0 = make_vec_env(FinEnv, n_envs=n_envs, monitor_dir=log_dir, vec_env_cls=SubprocVecEnv, 
            ...
            eval_env_1 = make_vec_env(FinEnv, n_envs=n_envs, monitor_dir=log_dir, vec_env_cls=SubprocVecEnv, 
            ...
            eval_env_2 = make_vec_env(FinEnv, n_envs=n_envs, monitor_dir=log_dir, vec_env_cls=SubprocVecEnv, 
            ...
            vec_env = make_vec_env(FinEnv, n_envs=n_envs, monitor_dir=log_dir, vec_env_cls=SubprocVecEnv, env_kwargs=env_kwargs)

            model = PPO("MlpPolicy", vec_env, tensorboard_log=tensorboard_dir_log,
            ...
            eval_env_0 = FinEnv(**eval_env_kwargs_0)
            ...
            eval_env_1 = FinEnv(**eval_env_kwargs_1)
            ...
            model.set_parameters(os.path.join(best_model_save_path, "best_model" + "_eval" + to_add))
            eval_env_2 = FinEnv(**eval_env_kwargs_2)
