...
def run_traj(env, reinforce_agent, max_steps, size):
    ...
    buffer=Buffer(n_steps = max_steps, obs_size=2*size+1+1)
    buffer.reset()
    obs, _, _ = env.reset()
    initial_cost=env.env.cost_list[np.argmax(obs[:-1])]
    ...
    min_cost_found = np.min(env.env.cost_list[np.argmax(buffer.observation[:,:-1], axis=1)])
    min_diff = np.min(np.abs(np.argmax(buffer.observation[:,:-1], axis=1)-env.env.size))
...
def run_traj_both(env, reinforce_agent, max_steps, size):
    ...
    buffer=Buffer(n_steps = max_steps, obs_size=2*size+1+1)
    buffer_normal=Buffer(n_steps = max_steps, obs_size=2*size+1+1)
    buffer.reset()
    obs, _, _ = env.reset()
    initial_cost=env.env.cost_list[np.argmax(obs[:-1])]
...
env = MaxWrapper(env_pill, adapt_state=True, adapt_reward=True)

# Setup agent

actor_net = Sequential([keras.Input(shape=(2*env.env.size+1+1,)), 
